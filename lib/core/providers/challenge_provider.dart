...
class ChallengeState {
  const ChallengeState({
    ...
    this.difficultyLevel = DifficultyLevel.level1,
    this.selectedAnswer,
  ...
  /// Niveli aktual i vshtirësisë — i lexueshëm nga UI.
...
class ChallengeConfig {
  ...
  @override
  int get hashCode => Object.hash(operation, difficultyLevel, sessionLength, identityHashCode(random));
}
...
class ChallengeNotifier extends StateNotifier<ChallengeState> {
  ...
  static MathQuestion _buildQuestion(ChallengeConfig cfg) {
    ...
    final maxNumber = cfg.difficultyLevel.maxNumber;
    late int num1;
    late int num2;
    late int answer;
    ...
    switch (cfg.operation) {
      ...
      case Operation.multiplication:
        final multMax = cfg.difficultyLevel == DifficultyLevel.level1
            ? 5
            : (cfg.difficultyLevel == DifficultyLevel.level2 ? 10 : 12);
        num1 = random.nextInt(multMax) + 1;
        num2 = random.nextInt(multMax) + 1;
        answer = num1 * num2;
      case Operation.division:
        final divMax = cfg.difficultyLevel == DifficultyLevel.level1
            ? 5
            : (cfg.difficultyLevel == DifficultyLevel.level2 ? 10 : 12);
