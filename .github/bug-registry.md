...
| B012 | Weekly stats ktheheshin bosh kur cloud mungonte, pa fallback Hive | **Lartë** | Fixed *(Sprint 14.1)* | `weekly_stats_provider.dart` |
| B013 | Dimensione hardcoded në `parent_report_screen.dart` (EdgeInsets, width statike) | **Mesatar** | Fixed *(Sprint 14.1)* | `parent_report_screen.dart` |
| B014 | Identifikuesit `uid`/`child.id` të plotë ekspozoheshin në loge sync | **Mesatar** | Fixed *(Sprint 14.1)* | `sync_service.dart` |
